...
// HttpClient creates an HTTP client from the given Google credentials and scopes.
func HttpClient(credentials *GcpCredentials, clientScopes ...string) (*http.Client, error) {
	var client *http.Client
	var err error
	if credentials == nil {
		ctx := context.WithValue(context.Background(), oauth2.HTTPClient, cleanhttp.DefaultClient())
		client, err = google.DefaultClient(ctx, clientScopes...)
		if err != nil {
			return nil, err
		}
	} else {
